...
class Firewall (EventMixin):
	...
	def _handle_PacketIn(self, event):
		...
		if(match.dl_type == packet.IP_TYPE):
		  if not self.port_security_check(packet, event, match):
                  	log.debug("possible flood detected, blocking flow")
	...
	def add_rule(self, smac, sip):
		if smac in self.blocked_addrs:
			if self.blocked_addrs.get(smac) == sip:
				return
		with open(l3config, 'a') as csvfile:
			log.debug("Reading l3 config file !")
		...
		self.blocked_addrs[smac] = sip


	def port_security_check(self, pkt, match, event):
		#check if mac in table
		pkt_i = pkt.payload
		if pkt.src in self.stored_msgs:
			#if mac in table then check ip spoofing
			if pkt_i.srcip != self.stored_msgs.get(pkt.src):
				#spoof attempt. block mac
				log.debug("Adding rule to block spoofing from mac: "+str(pkt.src))
				self.add_rule(str(pkt.src), 'any')
				return False

		else:
			#if mac not in table, add it. then check if ip spoofed (BONUS)
			if pkt_i.srcip in self.stored_msgs.values():
				#ip used my another mac. possible spoof
				log.debug("Adding rule to block spoofing from IP: "+str(pkt_i.srcip))
				self.add_rule('any', str(pkt_i.srcip))
				return False
			self.stored_msgs[pkt.src] = pkt_i.srcip
		return True
